...
export default function RoomSchedule({ schedules, roomId, onAddException }: RoomScheduleProps) {
  ...
  const getScheduleForCell = (date: Date, periodStart: string, periodEnd: string) => {
    ...
    const exception = timeMatches.find(s => 
      s.isException && s.effectiveFrom === dateStr
    )
    ...
    const regular = timeMatches.find(s => {
      ...
      const days = Array.isArray(s.weekdays) 
        ? s.weekdays 
        : String(s.weekdays || "").split(',').map(d => d.trim());
                  ...
                  {renderMatrix[pIndex].map((cell, dIndex) => {
                    ...
                    if (schedule) {
                        ...
                        return (
                          ...
                          >
                            <div className={`absolute inset-1 rounded-md p-1.5 text-[10px] flex flex-col gap-0.5 shadow-sm transition-all cursor-pointer border overflow-hidden
                                ${isCanceled 
